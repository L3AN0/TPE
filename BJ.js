...
var newGame = function() {
  ...
  if (handTotal(jsbApp.dealerHand) === 21) {
    ...
    jsbApp.textUpdates.innerHTML =
      "You lost! The dealer had 21 on their initial hand.";
    track();
    jsbApp.gameStatus = 2; // game is won
    return;
  }
...
var createDeck = function() {
  ...
  for (var a = 0; a < jsbApp.suits.length; a++) {
    for (var b = 0; b < jsbApp.values.length; b++) {
      ...
      if (cardValue != 1) {
        cardTitle +=
          jsbApp.values[b] + " of " + jsbApp.suits[a] + " (" + cardValue + ")";
      } else {
        cardTitle +=
          jsbApp.values[b] +
          " of " +
...
var drawHands = function() {
  ...
  if (jsbApp.gameStatus === 0) {
    ...
    jsbApp.dhandtext.innerHTML =
      "Dealer's Hand (" + jsbApp.dealerHand[1].value + " + hidden card)"; // hide value while a card is face down
  } else {
...
jsbApp.stayButton.addEventListener("click", function stayLoop() {
  ...
  if (jsbApp.gameStatus === 2) {
    ...
    if (handVal > 16 && handVal <= 21) {
      // dealer stays and game resolves
      drawHands();
...
var victory = function() {
  ...
  jsbApp.textUpdates.innerHTML =
    "You won!<br>" + explanation + "<br>Press 'New Game' to play again.";
  track();
};
...
var bust = function() {
  ...
  jsbApp.textUpdates.innerHTML =
    "You lost.<br>" + explanation + "<br>Press 'New Game' to play again.";
  track();
};
...
var tie = function() {
  ...
  jsbApp.textUpdates.innerHTML =
    "It's a tie at " +
    playerTotal +
    " points each.<br>Press 'New Game' to play again.";
  track();
};

// update the win/loss counter
var track = function() {
  jsbApp.tracker.innerHTML =
    "<p>Wins: " +
    jsbApp.wins +
    " Draws: " +
    jsbApp.draws +
